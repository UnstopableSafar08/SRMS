    ...
    <div class="main">
        <form action="" method="post">
            ...
            <legend>Enter Marks</legend>
                ...
                <input type="text" name="p1" id="" placeholder="Mark of Sub 1">
                <input type="text" name="p2" id="" placeholder="Mark of Sub 2">
                <input type="text" name="p3" id="" placeholder="Mark of Sub 3">
                <input type="text" name="p4" id="" placeholder="Mark of Sub 4">
                <input type="text" name="p5" id="" placeholder="Mark of Sub 5">
    ...
    if(isset($_POST['rno'],$_POST['p1'],$_POST['p2'],$_POST['p3'],$_POST['p4'],$_POST['p5']))
    {
       ...
       $name=mysqli_query($conn,"SELECT `name` FROM `students` WHERE `rno`='$rno' and `class_name`='$class_name'");
        while($row = mysqli_fetch_array($name)) {
         ...
         $query="SELECT * FROM result WHERE  result.class='$class_name' && rno='$rno'";
        $run=mysqli_query($conn,$query);
        if (mysqli_num_rows($run)>0) {

            echo "<script>alert('Results Already Exists !!!')</script>";
        }
else
    {
        ...
        if (!$sql) {
            ...
            echo 'alert("Student Data are not Exist on database. Invalid Details")';
            echo '</script>';
        }
        else{
            echo '<script language="javascript">';
